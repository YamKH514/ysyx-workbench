...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2); printf("SW, pc: %x, dnpc: %x, ADDR: 0x%x, val = 0x%x, src1 = 0x%x, imm = 0x%x, src2 = 0x%x\n", s->pc, s->dnpc, src1 + imm, src2, src1, imm, src2));
