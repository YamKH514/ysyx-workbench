...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + imm;
    IFDEF(CONFIG_FTRACE, {if(s->isa.inst == 0x00008067) {ftrace_ret(s->pc);}
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, BRANCH((sword_t)src1 < (sword_t)src2));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, BRANCH((sword_t)src1 >= (sword_t)src2));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , r, R(rd) = (src2 != 0) ? (sword_t)src1 % (sword_t)src2 : (sword_t)src1);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t old = 0; CSRR(); R(rd) = old; src1 = src1 | old; CSRW());
