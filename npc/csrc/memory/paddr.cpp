...
uint8_t *guest_to_host(uint32_t paddr) {
    if ((CONFIG_MBASE <= paddr) && (paddr <= CONFIG_MSIZE)) return pmem + paddr - CONFIG_MBASE;
    else if ((CONFIG_PSRAM_MBASE <= paddr) && (paddr <= CONFIG_PSRAM_MSIZE)) return psram + paddr - CONFIG_PSRAM_MBASE;
    assert(0);
}

uint32_t host_to_guest(uint8_t *haddr) {
    if ((CONFIG_MBASE <= (uintptr_t)haddr) && ((uintptr_t)haddr <= CONFIG_MSIZE)) return haddr - pmem + CONFIG_MBASE;
    else if ((CONFIG_PSRAM_MBASE <= (uintptr_t)haddr) && ((uintptr_t)haddr <= CONFIG_PSRAM_MSIZE)) return haddr - psram + CONFIG_PSRAM_MBASE;
    assert(0);
}
