...
extern "C" void mem_tracer_write(int32_t addr, int32_t data, int32_t strb)
{
    if ((CONFIG_SDRAMBASE <= addr) && (addr <= CONFIG_SDRAMBASE + CONFIG_SDRAMSIZE)) {
        if (strb && 1 << 0) pmem_write(addr+0, 1, (data >> 0));
        if (strb && 1 << 1) pmem_write(addr+1, 1, (data >> 8));
        if (strb && 1 << 2) pmem_write(addr+2, 1, (data >>16));
        if (strb && 1 << 3) pmem_write(addr+3, 1, (data >>24));
    }
#ifdef CONFIG_MTRACE
